...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	BoardDTO reView = (BoardDTO)request.getAttribute("boardReView");
%>
...
<div class="board_write_wrap">
	<div class="board_write_form">
		...
		<form action="boardReply.bo" method="post" name="boardForm" >
			<input type="hidden" name="board_num" value="<%=reView.getBoard_num() %>"/>
			<input type="hidden" name="board_re_ref" value="<%=reView.getBoard_re_ref() %>"/>
			<input type="hidden" name="board_re_lev" value="<%=reView.getBoard_re_lev() %>"/>
			<input type="hidden" name="board_re_seq" value="<%=reView.getBoard_re_seq() %>"/>
